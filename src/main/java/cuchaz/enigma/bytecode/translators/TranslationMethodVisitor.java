...
public class TranslationMethodVisitor extends MethodVisitor {
	...
	public void visitLocalVariable(String name, String desc, String signature, Label start, Label end, int index) {
		...
		if (argumentIndex >= 0) {
			LocalVariableDefEntry entry = new LocalVariableDefEntry(methodEntry, index, name, -1, new TypeDescriptor(desc));
	...
	public void visitEnd() {
		...
		if (!hasParameterMeta) {
			...
			for (int argumentIndex = 0; argumentIndex < arguments.size(); argumentIndex++) {
				LocalVariableEntry entry = new LocalVariableEntry(methodEntry, offset, "", -1);
